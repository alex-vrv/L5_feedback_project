...
# - L4  -> L2/3
#
# Additionally in this model, we add the connections
# - L5a -> PV
# - L5b -> L2/3
#
# Furthermore, the connection probability **is uniform**.
# - the ratio of L5a -> L2/3 to L5a -> PV connections is 1:1
# - the ratio of L5b -> L2/3 to L5b -> PV connections is 1:1
#
# L5a/b and L4 receive a step (exp_rise,exp_decay) (albeit noisy),
...
for iter in np.arange(8,9,1):

    #folder label indicating simulation details
    #notation: modeltype_stimulustype_activepopulationsorder

    #create set of unique combinations, since in the homogeneous model
    #L5a and L5b populations are identical
    pairs = combinations(np.arange(-100,110,10),2)
    pairs = list(pairs)
    iterations = []
    for locked_offset in np.arange(-100,110,10):
    ...
    for iteration in iterations:
        ...
        locked_offset = iteration[0][0]



        simulation_label = ''.join(('test_homogeneous_asynchronous_L5a',str(locked_offset),'_L5bvaried_temporaloffset_10ms_200mswindow_iter',str(iter)))
        if not os.path.exists("simulations/homogeneous_asynchronous/"+simulation_label):
            os.mkdir("simulations/homogeneous_asynchronous/"+simulation_label)
        ...
        for offset_pair in iteration:
            epochs.append('L5a '+str(locked_offset)+' L5b '+str(offset_pair[1])+' ms')
        ...
        for counter,offset in enumerate(iteration):
        ...
        max_I_timepointsL4 = find_peaks(I_rythmic_L4.values)[0]
        max_I_timepointsL5a = find_peaks(I_rythmic_L5a.values)[0]
        max_I_timepointsL5b = find_peaks(I_rythmic_L5b.values)[0]



        if save_plots == True:
            fig.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/stimulus.pdf')
        ...
        # print(max_I_timepointsL4)
        # print(max_I_timepointsL5a)
        # print(max_I_timepointsL5b)
        ...
        w_mean = dict()
        w_mean[('L5a', 'L2/3')] = 2. * b2.nsiemens
        w_mean[('L5b', 'L2/3')] = 2. * b2.nsiemens
        w_mean[('L5a', 'PV')  ] = 2. * b2.nsiemens
        w_mean[('L5b', 'PV')  ] = 2. * b2.nsiemens
        ...
        connections = dict()
        for (source_label, target_label) in [('L5a', 'L2/3'), ('L5a', 'PV'), ('L5b', 'L2/3'), ('L5b', 'PV')]:
        ...
        # Vogels-Sprekeler synapse to achieve an E/I balance
        ...
        # if save_plots == True:
        #     fig1.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/'+epochs[0]+'_firstspike.pdf')
        ...
        # if save_plots == True:
        #     fig2.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/'+epochs[1]+'_firstspike.pdf')
        ...
        # if save_plots == True:
        #     fig3.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/'+epochs[2]+'_firstspike.pdf')
        ...
        # if save_plots == True:
        #     fig.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/raster_all_populations.pdf')
        ...
        if save_plots == True:
            fig.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/FR_all_populations.pdf')
        ...
        # if save_plots == True:
        #     fig.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/single_L23_Vm.pdf')
        ...
        if save_plots==True:
            res.to_csv('simulations/homogeneous_asynchronous/'+simulation_label+'/population_peaks.csv')
        ...
        if save_plots == True:
            fig.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/FR_peakamp_hwhm_all.pdf')
        ...
        if save_plots==True:
            fig.savefig('simulations/homogeneous_asynchronous/'+simulation_label+'/L23_mean_response_curves.pdf')


        plt.close("all")


        print(''.join(('Simulation with ',str(locked_offset),' locked offset finished.')))
    print(''.join(('Iteration no.',str(iter),' completed.\n')))
